...
if 0
stillratiocut=0.9;min_sleep_dur=50;min_rem_dur=6;min_nonrem_dur=0;
...
cd(homedir);save('Rem_SW_sleep_analysis_2s','segall','remall','stillratiocut','s1all','s2all',...
...
%defaultcolor7=slanCM('guppy',7);
%defaultcolor7=slanCM('iceburn',7);0.4660 0.6740 0.1880
%defaultcolor7=slanCM('coolwarm',7);
...
cd(figdir);figure_title='fig1_thetapower_sleep';save_current_figure(figure_title);
%------------------------------------------------------------------------------------
% below look at csd during theta
close all;set(0,'DefaultFigureColormap',feval('turbo'));%clear all;in=1
homedir='X:\Mengni\Data_Analysis\Session_combined_0324';cd(homedir);load('SessionSet16');
load('Rem_SW_sleep_analysis','remall','nonremall');
factor=90;
