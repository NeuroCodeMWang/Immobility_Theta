function errbarplot(x,y,colorspec,ifphase,marker,legendstring)
...
if isstring(colorspec)
    hold on;errorbar(x,y,err,colorspec,'LineWidth',2);
else
    hold on;errorbar(x,y,err,'color',colorspec,'LineWidth',2);
end
%patch([x;flip(x)], [y-err;flip(y+err)], colorspec, 'FaceAlpha',0.3, 'EdgeColor','none');
if 0
legendstring1=[];
