function CALCULATE_PLACE_FIELDS_8ARMS_NPX_move_immobile_theta
...
ind=thetacycle(:,79)==0 & thetacycle(:,10)<10;
...
spike_trial(:,10)=0;Position_Data(:,7)=0;
for i=1:size(thetacycle,1)
    ind= spike_trial(:,1)>=thetacycle(i,22) & spike_trial(:,1)<=thetacycle(i,23) ;
    spike_trial(ind,10)=1;
    ind= Position_Data(:,1)>=thetacycle(i,22) & Position_Data(:,1)<=thetacycle(i,23) ;
    Position_Data(ind,7)=1;
end
...
Time_In_Position=zeros(binnum,binnum,7);
for i=1:binnum
    for j=1:binnum
        % movement
        ...
        % <10 cm/s, theta
        ind= positionbin(:,1)==i & positionbin(:,2)==j & Velocity<Velocity_Cutoff & Position_Data(:,7)==1;
        Time_In_Position(j,i,2)=sum(ind)*dt;
        % <10 cm/s, non theta non swr
        ind= positionbin(:,1)==i & positionbin(:,2)==j & Velocity<Velocity_Cutoff & Position_Data(:,7)==0;
        ...
        % <5 cm/s, theta
        ind= positionbin(:,1)==i & positionbin(:,2)==j & Velocity<5 & Position_Data(:,7)==1;
        Time_In_Position(j,i,4)=sum(ind)*dt;
        % <5 cm/s, non theta non swr
        ind= positionbin(:,1)==i & positionbin(:,2)==j & Velocity<5 & Position_Data(:,7)==0;
        indr= positionbin_ripple(:,1)==i & positionbin_ripple(:,2)==j & ripples(:,10)<5 ;
        Time_In_Position(j,i,5)=sum(ind)*dt-nansum(ripples(indr,2)-ripples(indr,1));
        % <1 cm/s, theta
        ind= positionbin(:,1)==i & positionbin(:,2)==j & Velocity<=1 & Position_Data(:,7)==1;
        Time_In_Position(j,i,6)=sum(ind)*dt;
        % <1 cm/s, non theta non swr
        ind= positionbin(:,1)==i & positionbin(:,2)==j & Velocity<=1 & Position_Data(:,7)==0;
        indr= positionbin_ripple(:,1)==i & positionbin_ripple(:,2)==j & ripples(:,10)<=1 ;
        Time_In_Position(j,i,7)=sum(ind)*dt-nansum(ripples(indr,2)-ripples(indr,1));
    end
end
...
Spikes_In_Position=zeros(binnum,binnum,size(unit_decode,1),7);
for N=1:size(spikedata,1)
    ...
    if spikedata(N,9)<Velocity_Cutoff
        Spikes_In_Position(spikedata(N,8),spikedata(N,7),spikedata(N,2),3-spikedata(N,10))=Spikes_In_Position(spikedata(N,8),spikedata(N,7),spikedata(N,2),3-spikedata(N,10))+1;
    end
    if spikedata(N,9)<5
        Spikes_In_Position(spikedata(N,8),spikedata(N,7),spikedata(N,2),5-spikedata(N,10))=Spikes_In_Position(spikedata(N,8),spikedata(N,7),spikedata(N,2),5-spikedata(N,10))+1;
    end
     if spikedata(N,9)<=1
        Spikes_In_Position(spikedata(N,8),spikedata(N,7),spikedata(N,2),7-spikedata(N,10))=Spikes_In_Position(spikedata(N,8),spikedata(N,7),spikedata(N,2),7-spikedata(N,10))+1;
    end
end
...
cd(savedir);save('Field_Data_move_immobile_theta','Field_Data','Field_Data_raw','field_property','unit_decode','spikedata','-v7.3');
...
for i=1:size(Field_Data,3)
    ...
    ind=spike(:,6)==i;
    subplot(1,3,3);histogram(spike(ind,4));title(unit_decode(i,10));
    pause(0.2);clf;
end
end